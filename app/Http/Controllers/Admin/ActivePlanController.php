...
class ActivePlanController extends Controller
{
    public function index($id)
    {
        ...
        $activePlans = PaymentPlan::join('active_plans', 'payment_plans.id', '=', 'active_plans.paymentPlanId')
            ...
            ->select('payment_plans.*', 'active_plans.*', 'users.*', 'active_plans.created_at AS startDate', 'payment_plans.name AS payment_name', 'payment_plans.duration_in_number AS remaining_days')->get();
        // dd($activePlans);
